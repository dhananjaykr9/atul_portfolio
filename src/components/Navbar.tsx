...
export default function Navbar() {
  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-[100] border-b border-oxford-blue/10 bg-ivory/95 shadow-[0_12px_35px_rgba(10,21,38,0.08)] backdrop-blur-xl transition-colors duration-300">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    prefetch
                    className={`rounded-full px-4 py-2.5 text-[11px] font-bold uppercase tracking-[0.22em] transition-all duration-300 ${isActive
      ...
      <div
        className={`overflow-hidden border-t border-oxford-blue/8 bg-white/95 backdrop-blur-xl transition-all duration-500 lg:hidden ${isOpen ? "max-h-[32rem] opacity-100" : "max-h-0 opacity-0"
          }`}
      >
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  prefetch
                  className={`flex items-center justify-between rounded-[22px] border px-4 py-4 transition-all duration-300 ${isActive
                ...
                >
                  <span className="flex items-center gap-3">
                    <span
                      className={`text-[10px] font-bold uppercase tracking-[0.25em] ${isActive ? "text-deep-gold/80" : "text-deep-gold"
                        }`}
                    >
                  ...
                  <span
                    className={`text-lg transition-transform duration-300 ${isActive ? "translate-x-0 text-deep-gold/80" : "text-oxford-blue/35"
                      }`}
                  >
                    &gt;
                  </span>
